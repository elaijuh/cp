...
class Solution {
 public:
  ...
  void traverse(vector<vector<int>>& image, int sr, int sc, int newColor) {
    if (sr < 0 || sr == h || sc < 0 || sc == w) return;
    if (image[sr][sc] == oldColor) {
      image[sr][sc] = newColor;
      for (vector<int>& d : direction)
        traverse(image, sr + d[0], sc + d[1], newColor);
    }
  }
  ...
  vector<vector<int>> floodFill(vector<vector<int>>& image, int sr, int sc,
    ...
    if (oldColor != newColor) traverse(image, sr, sc, newColor);
    return image;
  }
};
// @lc code=end
