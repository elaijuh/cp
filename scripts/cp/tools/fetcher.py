...
class LeetcodeFetcher:
    def __init__(self):
        ...
        self.algorithms_api_url = "https://leetcode.com/api/problems/algorithms/"
        self.headers = {
    ...
    def _get_slug_by_number(self, problem_number: int) -> str | None:
        try:
            response = requests.get(self.algorithms_api_url, headers=self.headers)
            if response.status_code == 200:
                data = response.json()
    ...
    def format_problem_info(self, problem_info: dict[str, Any]) -> dict[str, Any]:
        ...
        topics = [tag["slug"] for tag in problem_info.get("topicTags", [])]
        ...
        # Parse test cases
        test_cases = HTMLParser.parse_test_cases(
            problem_info.get("exampleTestcases", "")
        )
